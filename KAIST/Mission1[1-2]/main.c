...
{															  //
	initialize();											  //초기화
	int count;												  //판의 갯수 변수
	scanf("%d", &count);									  //판의 갯수 입력
	for (int i = 0; i < count; i++)							  //
	{														  //판의 갯수 만큼
		pole[0].plate[i] = count - i;						  //판의 갯수-(0~판의 갯수)->판의 갯수~1
	}														  //
	pole[0].plateNum = count - 1;							  // 0번째 기둥의 맨 위의 판의 번호는 갯수-1(시작이 0)
	printTower();											  //맨 처음 출력
	while (1)												  //
	{														  //무한 루프
...
{														   //
	for (int i = 0; i < 3; i++)							   // 3번(기둥의 갯수) 반복
	{													   //
		printf("%d:", i);								   //기둥 번호 출력
		if (pole[i].plateNum == -1)						   //판이 없을경우(-1)
			printf(" x");								   // x출력
		else											   //판이 있을경우
		{												   //
...
int move(int pole1, int pole2)											   // 이동 함수
{																		   //
