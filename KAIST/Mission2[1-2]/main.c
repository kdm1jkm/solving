...
void printSchedule(int titleLenth, int contentLenth, int cursorPlace); //일정 출력
...
{																																		//메인함수
	...
	while (1)																															//
	{																																	//무한루프
		printSchedule(titleLenth, contentLenth, cursorPlace);																			//그리기
		input = getch();																												//입력
		if (input == ARROW)																												//
		{																																//화살표일경우
			input = getch();																											//한번더 입력
...
void printSchedule(int titleLenth, int contentLenth, int cursorPlace)								//
{																									//일정 출력 함수
	system("cls");																					//화면 지우기
	int count = 0;																					//현재까지 출력한 일정의 갯수(커서 출력을 위해 필요)
	printf("  ┌─────┬───┬───┬──┬");																	//출력 틀(표)
