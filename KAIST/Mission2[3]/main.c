...
// 일정을 수정하는 함수를 추가하였다.
// 일정을 삭제하는 함수와 중요한 작업에는 되묻기를 추가햐였다.
// 그룹의 번호에 따른 이름:
// 0:공부
// 1:상담
// 2:약속
...
void deleteAllWithWarning();				//모든 일정을 경고후 삭제하는 함수
...
{																																										   //메인함수
	...
	{																																									   //무한루프
		...
		puts("[↓]밑으로    [↑]위로      [space]자세한 일정  [q]자세한 일정 나가기\n[e]일정 수정  [i]일정 추가  [d]일정 삭제        [o]프로그램 종료   [a]모든 일정 삭제"); //사용법 안내
		input = getch();																																				   //입력
		if (input == ARROW)																																				   //
		{																																								   //화살표일경우
			input = getch();																																			   //한번더 입력
		...
		{																																								   // d일경우
			printInformation(cursorPlace);																																   //자세한 일정 출력
			puts("\n\n정말로 삭제하시겠습니까? [y]네   [n]아니요");																										   //되묻기
			while (1)																																					   //
			{																																							   // y,n이 아닌키를 누르는상황에 대비
				input = getch();																																		   //입력
				if (input == 'y')																																		   //
		...
		{																																								   // o일경우
			system("cls");																																				   //화면지우기
			puts("정말로 종료하시겠습니까?   [y]네   [n]아니요");																										   //되묻기
			while (1)																																					   //
			{																																							   // y,n이 아닌키에 대비
				input = getch();																																		   //입력
				if (input == 'y')																																		   //
				{																																						   // y일경우
					printScheduleFile();																																   //마지막으로 혹시 모르니 파일로 내보내기
					goto end;																																			   //메모리 할당 해제를 위해
				}																																						   //
				else if (input == 'n')																																	   //
				{																																						   // n일경우
					break;																																				   // while문 나가기
		...
		{																																								   // a(모두삭제)일경우
			deleteAllWithWarning();																																		   //모두 삭제(함수 자체에서 2번 되물음)
			cursorPlace = 0;																																			   //커서는 0으로
			group[0].num = 1;																																			   // 0번그룹에 일정 추가(오류방지)
			group[0].schedule[0].year = 2017;																															   //
			group[0].schedule[0].month = 01;																															   //
			group[0].schedule[0].day = 01;																																   //
			group[0].schedule[0].hour = 9;																																   //
			group[0].schedule[0].min = 00;																																   //
			group[0].schedule[0].important = 9;																															   //
			group[0].schedule[0].title = realloc(group[0].schedule[0].title, sizeof(char) * strlen("none"));															   //
			strcpy(group[0].schedule[0].title, "none");																													   //
			group[0].schedule[0].content = realloc(group[0].schedule[0].content, sizeof(char) * strlen("none"));														   //
			strcpy(group[0].schedule[0].content, "none");																												   //
...
void deleteAllWithWarning()																	   //
{																							   //모든것을 지우는 함수
	system("cls");																			   //화면 지우기
	puts("정말로 모든 일정을 지울까요?\n지운 일정은 복구할 수 없습니다!   [y]예   [n]아니요"); //되묻기
	while (1)																				   //
	{																						   //
		int input;																			   //
		input = getch();																	   //
